...
const getProducts = (resultData: IProduct[]) => {
  products.value = resultData;
  const options: option[] = [];
  forEach(products.value, (item) => {
...
const filterChange = (filterValue:CheckboxValueType[]) => {
  ...
  for (let i = 0; i < filterValue.length; i += 1) {
    if (i === 0) {
      product = filterProduct(products.value, filterValue[i] as string);
    } else {
...
<template>
  <div class="allProducts">
    ...
    <el-drawer v-model="isOpen" direction="rtl" size="60%" :with-header="false" :show-close="false">
      <div v-for="title in Object.keys(checkboxLabel)" :key="title">
        <h4>{{ title }}</h4>
        <el-checkbox-group size="large" v-model="optionGroup" @change="filterChange">
          <el-checkbox
            style="display: block"
    ...
    <el-row class="productMain">
      ...
      <el-col class="right" :sm="21">
        <ProductList @getProducts="getProducts" :limit="limit"
        :newProduct="(optionGroup.length === 0 ? products : newProduct)"></ProductList>
      </el-col>
    </el-row>
  </div>
</template>
